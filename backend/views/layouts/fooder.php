...
<script>
    ...
    $(document).on('click', '.modalDelete', function(e){
     ...
     $(document).on('click', '.modalNotify', function(e){
        e.preventDefault();
         var url = $(this).val();
            $('.deletatag').attr("href", url);
            $('#customheader').html('<h4>Are you sure you want to <span style="color:blue;">send</span> notification ?</h4>');
            //$('#customheader').html('<span style="color:blue"> <i class="fa fa-bell"></i> Notification</span>');
            $('#textContent').html('<input type="text" class="form-control" style="width:200px;">');
            //$('#textContent').html('<h4>Are you sure you want to <span style="color:blue;">send</span> notification ?</h4>');
            $('#modal').modal('show').find('#modalContent').load();
    });
